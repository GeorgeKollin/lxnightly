...
class ConfigWin:
    def __init__(self):
        ...
        self.window = Tk()
        # Window title
        self.window.title("Linux Nightly v3.5 for f.lux")
        # Header image
        ...
        self.latin = Scale(self.window, from_=-90, to=90, orient=HORIZONTAL, resolution=1)
        ...
        self.lgtdin = Scale(self.window, from_=-179, to=180, orient=HORIZONTAL, resolution=1)
        ...
        self.zipin = Entry(self.window, width=9, textvariable=self.zipval)
        ...
        self.tmpin = Scale(self.window, from_=2000, to=10000, orient=HORIZONTAL, resolution=100)
        ...
        self.thmlist = Listbox(self.window, width=8, height=2, listvariable=self.thmlistinit)
        ...
        #self.nofin = IntVar()
        self.nofc = Checkbutton(self.window, text="[-nofork]", state="disabled", selectcolor="")
        ...
        self.iszipin = IntVar()
        self.iszipic = Checkbutton(self.window, text="Use ZIP Code instead of\nlatitude & longitude", variable=self.iszipin, selectcolor="")
        self.iszipic.grid(row=4, column=0, columnspan=2)
        if (str(zip2list[0]) == "1"):
            self.iszipic.select()
        # "Show command line output" checkbox widget
        self.chckout = IntVar()
        self.ifcmd = Checkbutton(self.window, text="Show command line output", variable=self.chckout, selectcolor="")
        self.ifcmd.grid(row=4, column=2, columnspan=2)
        # "Use daytime filter" checkbox widget
        self.fluxday = IntVar()
        self.forceday = Checkbutton(self.window, text="Use daytime filter (BETA) | Duration: 7 - 8 hours", variable=self.fluxday, selectcolor="")
        self.forceday.grid(row=5, column=0, columnspan=4)
        # "Turn off f.lux" button
        self.closebtn = Button(self.window, text="Turn off f.lux", command=self.killTask)
        self.closebtn.grid(row=6, column=0, pady=(5, 0), sticky="NSEW", columnspan=2)
        # "Exit" button
        self.extbtn = Button(self.window, text="Exit", command=self.window.destroy)
        self.extbtn.grid(row=6, column=2, pady=(5, 0), sticky="NSEW", columnspan=2)
        # "Apply changes" button
        self.startbtn = Button(self.window, text="Apply changes & turn on f.lux", command=self.fluxExec)
    ...
    def fluxExec(self):
        ...
        self.lgtdin.grid(row=1, column=3, pady=5)
        self.zipin = Entry(self.window, width=9, state="disabled", fg="black")
        self.zipin.grid(row=2, column=1, pady=5)
        ...
        if (thiscmd == "1"):
            ...
            self.yscroll = Scrollbar(self.window, orient=VERTICAL)
            self.yscroll.grid(row=9, column=3, pady=5, sticky="NSW")
            self.cmdtxt = Text(self.window, wrap=WORD, width=40, height=5, yscrollcommand=self.yscroll.set)
            self.cmdtxt.grid(row=9, column=0, padx=(5, 0), pady=5, sticky="E", columnspan=3)
            ...
            exetxt = Entry(self.window, textvariable=toexe, width=10, state="readonly", fg="black")
            exetxt.grid(row=10, column=2, pady=5, columnspan=2)
            copyr = Label(self.window, text="© 2020 Linux Nightly (v. 3.5.1343D7D).\nAll rights reserved.\nView source code on:")
            copyr.grid(row=11, column=0, pady=(10, 5), columnspan=4)
            txtgit = StringVar(value="https://github.com/georgekollin/lxnightly/")
            entgit = Entry(self.window, text=txtgit, state="readonly", width=45, fg="black")
        ...
        self.zipin = Entry(self.window, width=9, textvariable=zipvar)
        self.zipin.grid(row=2, column=1, pady=5)
        self.tmpin = Scale(self.window, from_=2000, to=10000, orient=HORIZONTAL, resolution=100)
        self.tmpin.grid(row=2, column=3, pady=5)
        ...
        if (thisrandr == "0"):
            ...
            self.randrc.select()
        #if(thisnof == "0"):
        #    self.nofc = Checkbutton(self.window, text="[-nofork]", variable=self.nofin, selectcolor="")
        #    self.nofc.grid(row=3, column=3, pady=5)
        #else:
        #    self.nofc = Checkbutton(self.window, text="[-nofork]", variable=self.nofin, selectcolor="")
        #    self.nofc.grid(row=3, column=3, pady=5)
